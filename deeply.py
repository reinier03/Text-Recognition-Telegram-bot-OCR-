        ...
        if clase_enviar and destinatario:
            if isinstance(clase_enviar, telebot.TeleBot):
                ...
                if res.status_code == 200:
                    ...
                    # Cuerpo del mensaje
                    cuerpo = MIMEText(res.json()["choices"][0]["message"]["content"].replace("**", "*").replace("!", "!"), 'plain', 'utf-8')
                    msg.attach(cuerpo)
                    ...
                    msg['From'] = EMAIL
                    msg['To'] = destinatario
                    msg['Subject'] = "Error con la IA"
                    
                    # Cuerpo del mensaje
                    cuerpo = MIMEText("Descripción del error:\n" + res.reason, 'plain', 'utf-8')
                    msg.attach(cuerpo)
